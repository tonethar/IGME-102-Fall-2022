...
### II-D. PE-17 - \*\*ERRATA\*\* - under "State constructor()"
 ...
 66.16 into the canvas breadth, with 100 pixels of padding left and right."
    - ***Correction: the minimum and maximum should be `-155.84` and `-66.3` respectively***
  - #2 - "Use the p5 `map()` function to rescale `this.y` to be between the minimum and maximum, -155.84 and - 66.3 into the canvas height, with 100 pixels of padding top and bottom."
    - ***Correction: the minimum and maximum should be `66.16` and `18.15` respectively***
...
- You must show the name of the state and it's population, as well as some other marker or symbol for that state
...
**(The top-left corner is Alaska, the bottom-left corner is Hawaii and the bottom-right corner is Puerteo Rico)**
